...
def handle_client(client_socket, other_device_socket, other_device_socket1):
    ...
    try:
        while True:
            ...
            if not data:
                break
            if not data_list[4, 0] == 0 and origin == 0:            # 取第一帧是否合适
            ...
            if not np.any(data_list == 0) or not number:
                # 除地形外其余特征
                Features[0, 0] = (data_list[0, 0] + data_list[1, 0]) / 2  # 质心y位置
                ...
                Features[0, 2:4] = [data_list[2, 0], (data_list[2, 1] - knee_terrain) / Height]
            ...
            other_device_socket1.send(send_fix)
        ...
        np.save('C:\\Users\\17543\\Desktop\\yangdata\\NPY\\0123night_1.npy', save_data)
        client_socket.close()
...
with h5py.File('LSTM_Params_V2.mat', 'r') as file:
...
muY = 14.9002
sigmaY = 15.6916
muX = np.array([[2.0123e+03, 0.5570, 2.0385e+03, 0.2833, 2.1391e+03, 0.5570, 5.6007e-05, 40.6117]])
sigmaX = np.array([[1.3809e+03, 0.0248, 1.3863e+03, 0.0240, 1.3915e+03, 0.0383, 1.4690, 29.4007]])
...
other_device_address1 = ('192.168.1.116', 8910)
other_device_socket1 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
other_device_socket1.connect(other_device_address1)
print('连接emg信号成功')
other_device_socket1.send('0')
...
try:
    while True:
        ...
        client_handler = threading.Thread(target=handle_client,
                                          args=(client_socket, other_device_socket,  other_device_socket1))
